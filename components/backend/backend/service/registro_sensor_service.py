...
class RegistroSensorService():
    ...
    def create_registro_sensor(esquema: Esquema, tipo_sensor: TipoSensor, zona_sensor: ZonaSensor, numero_sensor:int, valor:float, escala:str, nombre_planta:str = "Sin planta") -> CommonRegistroSensor:
        ...
        try:
            ...
            new_registro_sensor: RegistroSensor = RegistroSensorSet.create(session, tipo_sensor, zona_sensor, numero_sensor, valor, escala, nombre_planta)
            out= CommonRegistroSensor(new_registro_sensor.tipo_sensor,new_registro_sensor.zona_sensor,new_registro_sensor.numero_sensor,new_registro_sensor.valor, new_registro_sensor.escala, new_registro_sensor.nombre_planta, new_registro_sensor.id, new_registro_sensor.fecha)
    ...
    @staticmethod
    def create_registro_sensor_from_common(esquema: Esquema, registro_sensor: CommonRegistroSensor) -> CommonRegistroSensor:
        return RegistroSensorService.create_registro_sensor(esquema, registro_sensor.getTipoSensor(), registro_sensor.getNumeroSensor(), registro_sensor.getValor(), registro_sensor.getEscala(), registro_sensor.getNombrePlanta())

    @staticmethod
    def exists_registro_sensor(esquema: Esquema, id:int):
        session: Session = esquema.new_session()
    ...
    @staticmethod
    def list_registro_sensor(esquema: Esquema) -> List[CommonRegistroSensor]:
